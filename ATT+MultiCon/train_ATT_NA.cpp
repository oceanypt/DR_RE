...
float train_bags(string bags_name, string neg_bags_name)
{
	...
	for(map<int, string>:: iterator it = relations.begin(); it != relations.end(); it++)
	{
		...
		for (int k=0; k<bags_size; k++)
		{
			float s = 0;
			for (int i = 0; i < 3 * dimensionC; i++) 
				s += rList[k][i] * matrixRelationDao[r1 * 3 * dimensionC + i] * wt;   //e = x * A * r
			s = exp(s); 
			weight.push_back(s);
			weight_sum += s;
		}
		...
		{	
			...
			for (int i = 0; i < 3 * dimensionC; i++) 
				for (int k=0; k<bags_size; k++)
					r[i] += rList[k][i] * weight[k];
			float g = 0;
			if (relations.find(r2) != relations.end()){
				if (r2 == 0)
					g = 0;
				else
					g = -2 * exp(2 * (2.5 - f_r[r2])) / (1 + exp(2 * (2.5 - f_r[r2]))) * alpha1;	
			}
			else if(r2 == max_neg)
			{
				g = 2 * exp(2 * (0.5 + f_r[r2])) / (1 + exp(2 * (0.5 + f_r[r2]))) * alpha1;
			}
			for (int i = 0; i < 3 * dimensionC; i++) 
			{
				float g1 = 0;
